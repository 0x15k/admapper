...
from admapper.enum_pkg.ldap_users import enumerate_users_ldap
from admapper.enum_pkg.rid_cycle import cycle_rids
from admapper.enum_pkg.samr import enumerate_users_samr
...
def run_user_enumeration(session: Session) -> UserEnumResult:
    """Phase 2 — enumerate AD users from SAMR, LDAP, and RID cycling."""
    if session.workspace is None:
    ...
    print_phase(f"Phase 2 — user enumeration on {', '.join(targets)}")
