...
def test_pick_exploit_credential_prefers_verified_candidate(tmp_path) -> None:
    ...
    (ws / "password_candidates.json").write_text(
        '{"candidates": [{"username": "svc_user", "password": "KnownPassword123!", "verified": true}]}',
        encoding="utf-8",
    )
    ...
    cred = _pick_exploit_credential(session, "svc_user")
    assert cred is not None
    assert cred.secret == "KnownPassword123!"
...
def test_gssapi_modify_uses_faketime_subprocess_when_skew_set(tmp_path) -> None:
    ...
    with (
        ...
        ok, err = _modify_gmsa_membership_gssapi(
            ldap_host="dc01.target.example",
            gmsa_dn="CN=msa_target,CN=Managed Service Accounts,DC=target,DC=example",
